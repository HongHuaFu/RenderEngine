...
namespace Engine
{
	...
	PBR::PBR(Renderer* renderer)
	{
		...
		Shader* prefilterCapture  = Loader::LoadShader("pbr:prefilter", "shaders/pbr/cube_sample.vs", "shaders/pbr/prefilter_capture.fs");
		Shader* integrateBrdf     = Loader::LoadShader("pbr:integrate_brdf", "shaders/screen_quad.vs", "shaders/pbr/integrate_brdf.fs");
		...
		m_ProbeCaptureBackgroundShader = Loader::LoadShader("pbr:capture background", "shaders/capture_background.vs", "shaders/capture_background.fs");
		m_ProbeCaptureBackgroundShader->Use();
		m_ProbeCaptureBackgroundShader->SetInt("background", 0);

		// debug render
		m_ProbeDebugSphere = new Sphere(32, 32);
		m_ProbeDebugShader = Loader::LoadShader("pbr:probe_render", "shaders/pbr/probe_render.vs", "shaders/pbr/probe_render.fs");
	...
	std::vector<PBRCapture*> PBR::GetIrradianceProbes(glm::vec3 queryPos, float queryRadius)
	{
		...
		for (int i = 0; i < m_CaptureProbes.size(); ++i)
		{
			float lengthSq = ((m_CaptureProbes[i]->Position - queryPos)).length();
			lengthSq *= lengthSq;
