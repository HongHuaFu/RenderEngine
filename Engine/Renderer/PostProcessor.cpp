...
namespace Engine
{
	PostProcessor::PostProcessor(Renderer* renderer)
	{
		...
		{
			m_PostProcessShader = Loader::LoadShader("post process", "shaders/screen_quad.vs", "shaders/post_processing.fs");
		...
		// down sample
		{
			...
			m_DownSampleShader = Loader::LoadShader("down sample", "shaders/screen_quad.vs", "shaders/post/down_sample.fs");
		...
		// gaussian blur shader
		{
			...
			m_OnePassGaussianShader = Loader::LoadShader("gaussian blur", "shaders/screen_quad.vs", "shaders/post/blur_guassian.fs");
		...
		// ssao
		{
			...
			m_SSAOShader = Loader::LoadShader("ssao", "shaders/screen_quad.vs", "shaders/post/ssao.fs");
			m_SSAOShader->Use();
			...
			m_SSAONoise->Create(4, 4, GL_RGBA16F, GL_RGB, GL_HALF_FLOAT, &ssaoNoise[0]);
		...
		// bloom 
		{
			...
			m_BloomShader = Loader::LoadShader("bloom", "shaders/screen_quad.vs", "shaders/post/bloom.fs");
			m_SSAOShader->Use();
	...
	void PostProcessor::Blit(Renderer* renderer, Texture* source)
	{
		...
		// TODO: fix
		m_PostProcessShader->SetFloat("MotionScale", 20.0f / FPSTarget * 0.8);
