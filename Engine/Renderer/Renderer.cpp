...
namespace Engine
{
	...
	void Renderer::Init(GLADloadproc loadProcFunc)
	{
		...
		Texture *hdrMap = Loader::LoadHDR("sky env", "textures/backgrounds/alley.hdr");
		...
		if (Shadows)
		{
			...
			for (int i = 0; i < m_DirectionalLights.size(); ++i)
			{
				...
				if (light->CastShadows)
				{
					...
					glm::mat4 lightProjection = glm::orthoLH(-20.0f, 20.0f, 20.0f, -20.0f, -15.0f, 20.0f);

					glm::mat4 lightView = glm::lookAt(-light->Direction * 10.0f, glm::vec3(0.0), glm::vec3(0,1.0,0));
	...
	void Renderer::renderToCubemap(std::vector<RenderCommand>& renderCommands, TextureCube* target, glm::vec3 position, unsigned int mipLevel)
	{
		...
		float height = (float)target->m_FaceWidth * pow(0.5, mipLevel);
	...
	void Renderer::updateGlobalUBOs()
	{
		...
		glBufferSubData(GL_UNIFORM_BUFFER,   0, sizeof(glm::mat4), &(m_Camera->Projection * m_Camera->View)[0][0]); // sizeof(math::mat4) = 64 bytes
