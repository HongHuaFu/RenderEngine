...
namespace RE
{
	...
	// Ä¬ÈÏ³õÊ¼»¯
	void TextureCube::DefaultInitialize(unsigned int width, unsigned int height, unsigned int format, unsigned int type, bool mipmap)
	{
	...
	void TextureCube::GenerateFace(unsigned int face, unsigned int width, unsigned int height, unsigned int format, unsigned int type, unsigned char* data)
	{
		if(m_FaceWidth == 0)
			glGenTextures(1, &m_ID);
	...
	void TextureCube::SetMipFace(unsigned int face, unsigned int width, unsigned int height, unsigned int format, unsigned int type, unsigned int mipLevel, unsigned char *data)
	{
		Bind();
